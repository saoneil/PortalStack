...
function slimAthletesForClient(athletes) {
...
function sanitizeScheduleState(state) {
  ...
  (clone.catalog || []).forEach((entry) => {
    if (entry && typeof entry === 'object') {
      entry.athletes = slimAthletesForClient(entry.athletes);
    }
  });
  return clone;
}
...
function registerLiveScheduleRoutes(app, db, middleware) {
  const { requireLogin, requirePrincipleUser, liveScheduleLimiter } = middleware;
  ...
  function resolveCanEdit(req, clientId) {
    return Boolean(
      ...
      && Number(req.session.principleUser) === 1
    );
  }

  app.get('/api/live-schedule/saved', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.get('/api/live-schedule/:clientId/:eventId', liveScheduleLimiter, async (req, res) => {
    try {
      ...
      if (
        ...
        await new Promise((resolve) => {
          middleware.lookupUserFlags(req.session.username, req.session.clientId, (flags) => {
            ...
            canEdit = Number(flags.principleUser) === 1;
            resolve();
          });
        });
      }
      ...
      const catalog = (loaded.state && loaded.state.catalog) || [];
      const completed = await store.getCompletedScheduleDivisionIds(eventId, clientId, catalog);
      ...
      res.json({
        ...
        canEdit,
        timezone,
        completedDivisionIds: completed.ids || [],
        resultsUpdatedAt: completed.updatedAt || null
      });
    } catch (err) {
      console.error(err);
  ...
  app.put('/api/live-schedule/:clientId/:eventId', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const clientId = req.params.clientId;
      const eventId = req.params.eventId;
      ...
      const draws = await store.loadDraws(eventId, req.session.clientId);
      const hasDrawCatalog = (draws?.state?.catalog || []).some(
        (entry) => Number(entry.athlete_count || 0) > 0
      );
      if (!hasDrawCatalog) {
        await store.deleteSchedule(eventId, req.session.clientId);
        return res.status(409).json({
          error: 'Draws for this event were removed. Recreate draws in the draw builder, then open the schedule again.'
        });
      }
  ...
  app.post('/api/live-schedule/:clientId/:eventId/pack', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const clientId = req.params.clientId;
      const eventId = req.params.eventId;
      if (!clientIdsMatch(req.session.clientId, clientId)) {
