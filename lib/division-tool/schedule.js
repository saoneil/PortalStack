/**
 * Schedule packing for PMA Score — original implementation by Performance Martial Arts,
 * evolved from the author's earlier division-tool prototypes.
...
/** Latest clock time a day may auto-extend to before a new day is created. */
const MAX_DAY_END_MINUTES = 23 * 60 + 55;
const MAX_AUTO_DAYS = 31;
...
function matchCount(entry) {
  ...
  if (jsonData && typeof jsonData === 'object') {
    if (divisionType === 'Single Elimination' || divisionType === 'Round Robin') {
      const fromJson = (jsonData.matches || []).length;
      if (fromJson > 0) return fromJson;
    } else if (divisionType === 'Premier League') {
      ...
      const fromJson = poolMatches + elimMatches;
      if (fromJson > 0) return fromJson;
    }
  }
...
function occupiedIntervals(state, dayIndex, excludeId = null) {
  ...
  Object.entries(state.breaks || {}).forEach(([breakId, block]) => {
    ...
    if ((state.scratch_ids || []).map(String).includes(String(breakId))) return;
    const pDay = Number(block.day_index || 0);
    if (pDay !== dayIndex) return;
    const allRings = Boolean(block.all_rings);
    const ring = allRings ? 0 : Number(block.ring_index || 0);
    const span = allRings
      ? Math.max(1, ringCount)
      : Math.max(1, Number(block.ring_span || 1));
...
/** Extend a day's end_time toward end-of-day. Returns false when already at max. */
function tryExtendDayEnd(day, minutesNeeded) {
  if (!day || typeof day !== 'object') return false;
  const start = parseHhmm(day.start_time || DEFAULT_DAY.start_time) ?? 8 * 60;
  let end = parseHhmm(day.end_time || DEFAULT_DAY.end_time) ?? 18 * 60;
  if (end <= start) end += 24 * 60;
  if (end >= MAX_DAY_END_MINUTES) return false;
  const bump = Math.max(SCHEDULE_SLOT_MINUTES, snapMinutes(minutesNeeded || SCHEDULE_SLOT_MINUTES));
  const nextEnd = Math.min(MAX_DAY_END_MINUTES, end + bump);
  if (nextEnd <= end) return false;
  day.end_time = formatHhmm(nextEnd);
  return true;
}

/**
 * Find a slot, growing the schedule as needed:
 * 1) extend the current day's length (up to 23:55)
 * 2) if that day is fully extended, add another day and repeat
 */
function findPackSlotExpanding(state, entry, packOpts = {}, options = {}) {
  const allowNewDays = options.allowNewDays !== false;
  const maxAutoDays = Math.max(1, Number(options.maxAutoDays) || MAX_AUTO_DAYS);
  const duration = displayDurationMinutes(entry, state.match_durations, state.buffer_durations);
  if (duration <= 0) return null;

  if (!Array.isArray(state.days) || !state.days.length) {
    state.days = [{ ...DEFAULT_DAY }];
  }

  for (let attempt = 0; attempt < 500; attempt += 1) {
    const slot = findPackSlot(state, entry, packOpts);
    if (slot) return slot;

    const days = state.days;
    const rangeStart = Math.max(0, Number(packOpts.startDayIndex) || 0);
    const rangeEnd = packOpts.endDayIndex == null
      ? days.length - 1
      : Math.min(Number(packOpts.endDayIndex), days.length - 1);
    const targetDay = days[Math.max(rangeStart, rangeEnd)];
    const need = duration + SCHEDULE_MIN_GAP_MINUTES;

    if (targetDay && tryExtendDayEnd(targetDay, need)) {
      continue;
    }

    if (!allowNewDays || days.length >= maxAutoDays) {
      return null;
    }
...
function placeDivisionIds(state, divisionIds, {
  ...
  const dayStart = Math.max(0, Math.min(Number(startDayIndex) || 0, next.days.length - 1));
  ...
  divisionIds.forEach((rawId) => {
    ...
    // Ensure durations exist so every athlete division can be timed on the board.
    next.match_durations = next.match_durations || {};
    next.buffer_durations = next.buffer_durations || {};
    if (!(divisionId in next.match_durations) || !Number(next.match_durations[divisionId])) {
      next.match_durations[divisionId] = DEFAULT_MATCH_MINUTES;
    }
    if (!(divisionId in next.buffer_durations)) {
      next.buffer_durations[divisionId] = DEFAULT_BUFFER_MINUTES;
    }

    if (displayDurationMinutes(entry, next.match_durations, next.buffer_durations) <= 0) {
      if (!Array.isArray(next.scratch_ids)) next.scratch_ids = [];
      if (!next.scratch_ids.includes(divisionId)) next.scratch_ids.push(divisionId);
      skipped += 1;
      return;
    }
    ...
    let slot = expandDays
      ? findPackSlotExpanding(next, entry, packOpts, {
        allowNewDays: !dayOnly,
        maxAutoDays: MAX_AUTO_DAYS
      })
      : findPackSlot(next, entry, packOpts);
...
function createScheduleFromDraws(drawsState, options = {}) {
  ...
  const packed = placeDivisionIds(state, ids, { replaceExisting: true, expandDays: true });
  return {
    state: packed.state,
...
function syncScheduleFromDraws(existingState, drawsState, options = {}) {
  ...
  const packed = placeDivisionIds(next, toPack, { replaceExisting: false, expandDays: true });
  return {
    state: packed.state,
    placed: packed.placed,
    skipped: packed.skipped
...
function boardRowsForDay(state, dayIndex = 0) {
  ...
  Object.entries(state.breaks || {}).forEach(([breakId, block]) => {
    if ((state.scratch_ids || []).map(String).includes(String(breakId))) return;
    if (Number(block.day_index || 0) !== dayIndex) return;
    const duration = Math.max(SCHEDULE_SLOT_MINUTES, snapMinutes(Number(block.duration_minutes || 0)));
    const start = Number(block.start_offset_minutes || 0);
    const allRings = Boolean(block.all_rings);
    const ring = allRings ? 0 : Number(block.ring_index || 0);
    ...
    rows.push({
      kind: 'break',
      id: breakId,
      name: allRings ? 'Break · all rings' : 'Break',
      type: 'Break',
      event_key: '',
      athletes: 0,
      ring,
