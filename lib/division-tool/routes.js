...
function registerDivisionAdvancedRoutes(app, db, middleware) {
  const { requireLogin, requirePrincipleUser } = middleware;
  ...
  async function syncEventScheduleFromDraws(eventId, clientId, drawsState) {
    ...
    const hasDrawableCatalog = drawsState.catalog.some(
      (entry) => Number(entry.athlete_count || 0) > 0
    );
    if (!hasDrawableCatalog) {
      try {
        const removed = await store.deleteSchedule(eventId, clientId);
        return { scheduleSynced: true, scheduleDeleted: true, schedulesRemoved: removed };
      } catch (err) {
        console.error('schedule delete after empty draws failed:', err);
        return { scheduleSynced: false, scheduleSyncError: err.message || 'schedule delete failed' };
      }
    }
    try {
  ...
  app.get('/api/division-advanced/events', requireLogin, requirePrincipleUser, (req, res) => {
  ...
  app.get('/api/division-advanced/divisions/templates', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.get('/api/division-advanced/divisions/templates/:templateId', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.post('/api/division-advanced/divisions/templates', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.delete('/api/division-advanced/divisions/templates/:templateId', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.post('/api/division-advanced/divisions/create-all-defaults', requireLogin, requirePrincipleUser, (req, res) => {
    const { leaves, failures, count } = createAllDefaultDivisions();
    res.json({ leaves, failures, count });
  });

  app.get('/api/division-advanced/divisions/pattern-defaults', requireLogin, requirePrincipleUser, (req, res) => {
  ...
  app.post('/api/division-advanced/divisions/generate-pattern', requireLogin, requirePrincipleUser, (req, res) => {
    try {
      const body = req.body || {};
  ...
  app.post('/api/division-advanced/events/:eventId/divisions', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.get('/api/division-advanced/events/:eventId/divisions', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.get('/api/division-advanced/events/:eventId/creation-status', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.get('/api/division-advanced/events/:eventId/draws', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.get('/api/division-advanced/events/:eventId/athletes', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
      ...
      const payload = athletes.map((athlete) => {
        const snap = athleteSnapshot(athlete);
        const events = {};
        EVENT_COLUMNS.forEach((key) => {
          events[key] = Number(athlete[key] || 0) === 1 ? 1 : 0;
        });
        return { ...snap, events };
      });
      res.json({ athletes: payload });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: err.message || 'Unable to load athletes.' });
    }
  });

  app.put('/api/division-advanced/events/:eventId/draws', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.post('/api/division-advanced/events/:eventId/draws/create-from-divisions', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.post('/api/division-advanced/events/:eventId/draws/move', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.post('/api/division-advanced/draws/edit', requireLogin, requirePrincipleUser, (req, res) => {
    try {
  ...
  app.get('/api/division-advanced/events/:eventId/draws/pdfs.zip', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.get('/api/division-advanced/results/events', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const clientId = req.session.clientId;
  ...
  app.get('/api/division-advanced/events/:eventId/results/meta', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
  ...
  app.get('/api/division-advanced/events/:eventId/results.pdf', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.post('/api/division-advanced/events/:eventId/draws/regenerate', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
  ...
  app.post('/api/division-advanced/events/:eventId/draws/preview-entry', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
      const eventId = req.params.eventId;
      const clientId = req.session.clientId;
      const owned = await store.verifyEventOwnership(eventId, clientId);
      if (!owned) return res.status(403).json({ error: 'Access denied.' });
      const incoming = req.body?.entry;
      ...
      const athletes = normalizeAthletesFromRows(rows);
      const next = {
        id: String(incoming.id || '').trim() || `custom_${Date.now()}`,
        division_name: String(incoming.division_name || '').trim(),
        division_type: String(incoming.division_type || 'Single Elimination').trim(),
        event_key: String(incoming.event_key || '').trim(),
        athlete_indices: Array.isArray(incoming.athlete_indices)
          ? incoming.athlete_indices.map((x) => Number(x)).filter((x) => Number.isFinite(x))
          : []
      };
      rebuildDrawCatalogEntry(next, athletes);
      res.json({ entry: next });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: err.message || 'Unable to preview custom draw.' });
    }
  });

  app.delete('/api/division-advanced/events/:eventId/profile-files', requireLogin, requirePrincipleUser, async (req, res) => {
    try {
  ...
  app.get('/api/division-advanced/meta', requireLogin, requirePrincipleUser, (req, res) => {
    res.json({
