...
function createDrawsFromGroupings(groupingsState, options = {}) {
  ...
  const { stateSignature } = require('./utils');
  const catalog = [];
  ...
  (groupingsState.catalog || []).forEach((entry) => {
    ...
    if (!competitors.length) {
      catalog.push({
        ...
        division_type: entry.division_type || 'List',
  ...
  return {
    ...
    groupings_signature_at_create: groupingsState.signature || stateSignature(groupingsState),
    groupings_out_of_sync: false,
    catalog
  };
}
